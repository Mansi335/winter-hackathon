...
export default function AssistiveHub() {
  ...
  const emergencyOptions = [
    { id: 'help', symbol: '🆘', text: 'I need help', color: 'bg-red-500' },
    { id: 'medical', symbol: '🏥', text: 'Medical emergency', color: 'bg-red-600' },
    { id: 'police', symbol: '🚔', text: 'Call police', color: 'bg-blue-500' },
    { id: 'fire', symbol: '🔥', text: 'Fire emergency', color: 'bg-orange-500' },
    { id: 'water', symbol: '💧', text: 'I need water', color: 'bg-blue-400' },
    { id: 'bathroom', symbol: '🚽', text: 'Bathroom', color: 'bg-green-500' },
    { id: 'hungry', symbol: '🍽️', text: 'I am hungry', color: 'bg-yellow-500' },
    { id: 'pain', symbol: '😣', text: 'I am in pain', color: 'bg-red-400' },
  ];

  const handleEmergencySelect = (id: string) => {
    setSelectedEmergency(id);
    // In production, this would trigger an alert or notification
    setTimeout(() => {
      alert(`Emergency message sent: ${emergencyOptions.find(opt => opt.id === id)?.text}`);
    }, 500);
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-lg mb-8">
          <div className="flex flex-wrap border-b border-gray-200">
            {[
              { id: 'sign-language', label: 'Sign Language Translator', icon: '👋' },
              { id: 'speech-text', label: 'Speech-to-Text', icon: '🎤' },
              { id: 'image-desc', label: 'Image Description', icon: '🖼️' },
              { id: 'emergency', label: 'Emergency Communication', icon: '🆘' },
            ].map((tab) => (
              <button
                key={tab.id}
            ...
            {activeTab === 'emergency' && (
              <div className="space-y-6">
                ...
                <p className="text-gray-600 mb-6">
                  One-tap communication using visual symbols instead of words
                </p>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {emergencyOptions.map((option) => (
                    <button
                      ...
                      className={`${option.color} text-white rounded-lg p-6 hover:opacity-90 transition-all transform hover:scale-105 shadow-lg ${
                        selectedEmergency === option.id ? 'ring-4 ring-yellow-400' : ''
                      }`}
                    >
                ...
                {selectedEmergency && (
                  <div className="bg-green-100 border border-green-300 rounded-lg p-4 mt-6">
                    <p className="text-green-800 font-medium">
                      ✓ Emergency message sent: {emergencyOptions.find(opt => opt.id === selectedEmergency)?.text}
                    </p>
                  </div>
                )}
              </div>
